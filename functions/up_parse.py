...
class Up_parse:
    ...
    def __init__(self, path_to_fits,off_set=0.0):
        ...
        self.wave_ext  = 0.0 # reserved for future
...
#        self.weight = np.array([1.0 if mask else 1E-10 for mask in self.mask]) # no working in spline?
    ...
    def get_wl_array(self,file_list):
        """
        The module is to determine an extended wavelength array.
        
        It takes wl arrays of all the spectra and build an array which covers them all.
        """
        begin = []
        end = []
        for path in file_list:
            f = fits.open(path)
            begin.append(f[0].header['UP_WLSRT'])
            end.append(f[0].header['UP_WLEND'])
        f.close()
        an_begin = self.wave[0]
        an_end   = self.wave[-1]
        begin_w = np.min(begin)
        end_w   = np.max(end)
        if begin_w < an_begin:
            wl_0     = 10.0**self.fits_opened[0].header['CRVAL1']
            CRPIX1   = self.fits_opened[0].header['CRPIX1']
            log_disp = self.fits_opened[0].header['CD1_1']
            # find the pixel corresponding to begin_w
            index  = int(round(np.log10(begin_w/wl_0)/log_disp + CRPIX1 - 1.))
            factor = self.w_shift*wl_0*10.0**((1-CRPIX1)*log_disp) # normally equals to w_shift*wl_0
            begin_array = np.array([factor*10.0**(i*log_disp) for i in np.arange(index,0)])
        else:
            begin_array = []
        if end_w > an_end:
            wl_0     = 10.0**self.fits_opened[0].header['CRVAL1']
            CRPIX1   = self.fits_opened[0].header['CRPIX1']
            log_disp = self.fits_opened[0].header['CD1_1']
            # find the pixel corresponding to end_w
            index  = int(round(np.log10(end_w/wl_0)/log_disp + CRPIX1 - 1.))
            factor = self.w_shift*wl_0*10.0**((1-CRPIX1)*log_disp) # normally equals to w_shift*wl_0
            end_array = np.array([factor*10.0**(i*log_disp) for i in np.arange(len(self.wave),index+1)])
        else:
            end_array = []
        self.wave_ext = np.concatenate((begin_array,self.wave,end_array))
